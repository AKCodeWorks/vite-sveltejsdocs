...
type HighlightLanguage = 'javascript' | 'json' | 'typescript';
...
export const load: PageServerLoad = async ({ params }) => {
	...
	const highlightedExamples: HighlightedExample[] = await Promise.all(
		rawExamples.map(async (example) => ({
			key: example.key,
			label: example.label,
			html: await highlightCode(example.content, 'javascript')
		}))
	);
...
export const createDocsSlugPageSvelteModule = (collapseJson = false): string => {
	...
	const nonAuthorTags = $derived(
		data.doc.tags.filter((tag) => {
			...
			return normalized !== 'author' && normalized !== 'method' && normalized !== 'headers' && normalized !== 'body';
		})
	);
	...
	>
			...
			<section class="docs-detail-section mt-6 border-t border-zinc-200 pt-6">
				...
				<ul class="mt-3 space-y-2">
					{#each data.doc.params as param (\`\${param.name}-\${param.value}\`)}
						...
						>
							<code class="mr-2 inline-block border border-cyan-300 bg-cyan-100 px-1.5 py-0.5 font-semibold text-cyan-800">@{param.name}</code>
							{#if isStructuredValue(param.value)}
								<pre class="mt-2 overflow-x-auto text-xs text-zinc-950"><code>{@html renderStructuredValue(param.value)}</code></pre>
							{:else}
								{param.value}
							{/if}
						</li>
		...
		{#if data.doc.returns}
			<section class="docs-detail-section mt-8 border-t border-zinc-200 pt-6">
				...
				{:else if isStructuredValue(data.doc.returns)}
